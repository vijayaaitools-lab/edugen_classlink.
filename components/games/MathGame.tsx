...
import { mathGames, type GameTopic } from "@/lib/game-data";
...
export default function MathGame() {
  ...
  }, [selected, selectedTopic, answers, currentQ]);
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-blue-700">Mathematics</h2>
          <p className="text-gray-500 mt-1">Choose a topic to practice</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {mathGames.topics.map((topic, i) => (
            <button
              key={topic.id}
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-blue-600 font-bold">{selectedTopic.icon}</span>
            <span className="font-semibold text-blue-700">{selectedTopic.name}</span>
          </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          <div className="text-xs text-blue-500 font-medium mb-2">Question {currentQ + 1} of {total}</div>
          <p className="text-gray-800 font-semibold text-lg">{q.question}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {q.options.map((opt, idx) => {
            let cls = "border-2 border-gray-200 bg-white hover:border-blue-400 hover:bg-blue-50";
            if (showExplanation) {
            ...
            return (
              ...
              >
                <span className="mr-2 text-blue-400 font-bold">{["A", "B", "C", "D"][idx]}.</span>
                {opt}
              </button>
            );
          })}
    ...
    return (
      <div className="text-center space-y-6 py-4">
        ...
        <div className="bg-blue-50 rounded-2xl p-6 inline-block">
          <div className="text-5xl font-bold text-blue-600">{score}/{total}</div>
          <div className="text-gray-500 mt-1">Score • {pct}%</div>
        </div>
        <div className="flex gap-3 justify-center">
          <Button onClick={() => startGame(selectedTopic)} className="bg-blue-600 hover:bg-blue-700">
            Try Again
          </Button>
