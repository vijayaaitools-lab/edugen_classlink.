...
import { hindiGames, type GameTopic } from "@/lib/game-data";
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-orange-700">Hindi / हिंदी</h2>
          <p className="text-gray-500 mt-1">अभ्यास के लिए विषय चुनें</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {hindiGames.topics.map((topic, i) => (
            <button
              key={topic.id}
            ...
            >
              <div className="text-3xl mb-2 font-bold">{topic.icon}</div>
              <div className="font-semibold text-sm leading-tight">{topic.name}</div>
              <div className="text-xs opacity-80 mt-1">{topic.questions.length} प्रश्न</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-orange-700">{selectedTopic.name}</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className={cn(
              ...
              timeLeft <= 10 ? "bg-red-500 animate-pulse" : "bg-orange-500"
            )}>
              {timeLeft}
            </div>
          </div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
          <div className="text-xs text-orange-500 font-medium mb-2">प्रश्न {currentQ + 1} / {total}</div>
          <p className="text-gray-800 font-semibold text-lg leading-relaxed">{q.question}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {q.options.map((opt, idx) => {
            let cls = "border-2 border-gray-200 bg-white hover:border-orange-400 hover:bg-orange-50";
            if (showExplanation) {
            ...
            return (
              <button
                ...
                className={cn(
                  "rounded-xl p-3 text-left transition-all font-medium text-sm leading-relaxed",
                  cls,
                  !showExplanation && "cursor-pointer"
                )}
              >
                <span className="mr-2 text-orange-500 font-bold">{["क", "ख", "ग", "घ"][idx]}.</span>
                {opt}
              </button>
            );
          })}
      ...
      <div className="text-center space-y-6 py-4">
        ...
        <div>
          <h3 className="text-2xl font-bold text-orange-700">
            {pct >= 80 ? "शाबाश! बहुत बढ़िया!" : pct >= 50 ? "अच्छा किया!" : "अभ्यास जारी रखें!"}
          </h3>
          <p className="text-gray-500 mt-1">{selectedTopic.name} पूरा हुआ</p>
        </div>
        <div className="bg-orange-50 rounded-2xl p-6 inline-block">
          <div className="text-5xl font-bold text-orange-600">{score}/{total}</div>
          <div className="text-gray-500 mt-1">अंक • {pct}%</div>
        </div>
        <div className="flex gap-3 justify-center">
          <Button onClick={() => startGame(selectedTopic)} className="bg-orange-600 hover:bg-orange-700">
            फिर से खेलें
          </Button>
