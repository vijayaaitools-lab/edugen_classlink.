...
import { scienceGames, type GameTopic } from "@/lib/game-data";
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-purple-700">Science</h2>
          <p className="text-gray-500 mt-1">Choose a topic to explore</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {scienceGames.topics.map((topic, i) => (
            <button
              key={topic.id}
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-purple-700">{selectedTopic.icon} {selectedTopic.name}</span>
          </div>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-5">
          <div className="text-xs text-purple-500 font-medium mb-2">Question {currentQ + 1} of {total}</div>
          <p className="text-gray-800 font-semibold text-lg">{q.question}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {q.options.map((opt, idx) => {
            let cls = "border-2 border-gray-200 bg-white hover:border-purple-400 hover:bg-purple-50";
            if (showExplanation) {
            ...
            return (
              ...
              >
                <span className="mr-2 text-purple-500 font-bold">{["A", "B", "C", "D"][idx]}.</span>
                {opt}
              </button>
            );
          })}
      ...
      <div className="text-center space-y-6 py-4">
        ...
        <div>
          <h3 className="text-2xl font-bold text-purple-700">
            {pct >= 80 ? "Brilliant!" : pct >= 50 ? "Good Work!" : "Keep Exploring!"}
          </h3>
          <p className="text-gray-500 mt-1">{selectedTopic.name} Complete</p>
        </div>
        <div className="bg-purple-50 rounded-2xl p-6 inline-block">
          <div className="text-5xl font-bold text-purple-600">{score}/{total}</div>
          <div className="text-gray-500 mt-1">Score • {pct}%</div>
        </div>
        <div className="flex gap-3 justify-center">
          <Button onClick={() => startGame(selectedTopic)} className="bg-purple-600 hover:bg-purple-700">
            Try Again
          </Button>
